...
//  TodoApp_CoreData
//
//  Created by Designer on 2023/07/04.
//
...
class CoreDataManager {
    ...
    func create<T: NSManagedObject>(entity: T.Type, completion: (T) -> Void) {
        ...
        guard let entityDescription = NSEntityDescription.entity(forEntityName: T.self.description(), in: context) else {
            return
        }
        guard let managedObject = NSManagedObject(entity: entityDescription, insertInto: context) as? T else {
            return
        }
